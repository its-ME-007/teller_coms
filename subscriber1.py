...
def on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        ...
        topics = [("message_from_2", 0), ("message_from_3", 0), ("message_from_4", 0)]
...
def main():
    ...
    if using_v2:
        client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, "user_1")
    else:
    ...
    # Set up will message
    client.will_set("user_status", payload=json.dumps({"status": "offline", "user": "user_1"}), qos=1, retain=True)
    ...
    while True:
        try:
            ...
            client.publish("message_from_1", payload_json)
            print(f"Published from user1: {payload_json}")
